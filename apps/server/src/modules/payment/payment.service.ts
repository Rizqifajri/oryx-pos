 ...
 */
export const createPublicSnapToken = async (orderId: string) => {
  ...
  if (order.status === "COMPLETED") {
    const existingTx = await transactionRepo.findTransactionByOrderId(orderId);
    if (existingTx) {
  ...
  const existing = await paymentRepo.findPaymentRequestByOrderId(orderId);
  ...
  const orderItems = await orderRepo.findOrderItemsByOrderId(orderId);
  ...
  const paymentRequest = await paymentRepo.createPaymentRequest({
 ...
 */
export const createSnapToken = async (ctx: UserContext, orderId: string) => {
  // Get order with full details
  const order = await orderRepo.findOrderById(orderId);
  if (!order) throw new AppError("Order not found", 404);
 ...
 */
export const handleWebhook = async (notificationData: MidtransWebhookPayload) => {
  ...
  // Get payment request
  const paymentRequest = await paymentRepo.findPaymentRequestByMidtransOrderId(
  ...
  // Get order
  const order = await orderRepo.findOrderById(paymentRequest.orderId);
  if (!order) throw new AppError("Order not found", 404);
  ...
  await paymentRepo.updatePaymentRequestStatus(paymentRequest.id, {
  ...
  if (paymentStatus === "success") {
    ...
    await db.transaction(async (tx) => {
      ...
      const existingTx = await transactionRepo.findTransactionByOrderId(
        order.id,
      );
      if (existingTx) {
 ...
 */
export const getPaymentRequest = async (ctx: UserContext, id: string) => {
  const paymentRequest = await paymentRepo.findPaymentRequestById(id);
 ...
 */
export const getPaymentRequestByOrder = async (
  ctx: UserContext,
  orderId: string,
) => {
  const order = await orderRepo.findOrderById(orderId);
  ...
  const paymentRequest = await paymentRepo.findPaymentRequestByOrderId(orderId);
  if (!paymentRequest) {
...
export const listPaymentRequests = async (
  ...
  if (ctx.scope === "TENANT") {
    if (!ctx.tenantId) throw new AppError("Tenant context required", 400);
    return await paymentRepo.findPaymentRequestsByTenantId(ctx.tenantId);
  }

  // For GLOBAL scope: use filter if provided
  if (filters?.tenantId) {
    return await paymentRepo.findPaymentRequestsByTenantId(filters.tenantId);
  }
