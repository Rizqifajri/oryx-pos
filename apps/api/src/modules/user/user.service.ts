...
export class UserService {
  ...
  async listUsers(ctx: UserContext) {
    ...
    if (ctx.scope === "TENANT") {
      ...
      return await this.userRepo.findUsersByTenantId(ctx.tenantId);
    }

    return await this.userRepo.findAllUsers();
  }

  async getUsersByTenant(ctx: UserContext, tenantId: string) {
    // Permission check now handled by route middleware
    if (ctx.scope === "TENANT") {
      if (!ctx.tenantId) {
        throw new AppError("Tenant context required", 400);
    ...
    return await this.userRepo.findUsersByTenantId(tenantId);
  }
  ...
  async createUser(ctx: UserContext, input: CreateUserInput) {
    // Permission check now handled by route middleware

    // First, fetch the role to determine scope
    const role = await this.roleRepo.findRoleById(input.roleId);
    if (!role) {
    ...
    if (role.scope === "GLOBAL") {
      ...
      if (ctx.scope === "TENANT") {
        ...
        if (input.tenantId !== ctx.tenantId) {
          throw new AppError(
            "You can only create users in your own tenant",
            403,
          );
        }
  ...
  async updateUser(ctx: UserContext, id: string, input: UpdateUserInput) {
    // Permission check now handled by route middleware
    const user = await this.userRepo.findUserById(id);
    if (!user) {
      throw new AppError("User not found", 404);
  ...
  async deleteUser(ctx: UserContext, id: string) {
    // Permission check now handled by route middleware
    const user = await this.userRepo.findUserById(id);
    if (!user) {
      throw new AppError("User not found", 404);
