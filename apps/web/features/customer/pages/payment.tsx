...
import { ArrowLeft, CreditCard, Wallet, CheckCircle2, QrCode, Receipt, Loader2 } from "lucide-react"
import { Button } from "@/components/ui/button"

// Simulasi data pesanan yang dioper (bisa dari state / cache)
const DUMMY_ORDER_SUMMARY = {
  orderId: "ORD-2026-8891",
  tableId: "04",
  items: [
    { id: "m1", name: "Nasi Goreng Spesial", quantity: 2, price: 3500000 },
    { id: "m4", name: "Es Teh Manis", quantity: 2, price: 800000 },
  ],
  tax: 780000,      // Pajak PB1 10%
  service: 390000,  // Service charge 5%
}
...
export function CustomerPaymentPage() {
  ...
  // Hitung Kalkulasi Struk
  const subtotal = DUMMY_ORDER_SUMMARY.items.reduce((sum, item) => sum + item.price * item.quantity, 0)
  const totalPayment = subtotal + DUMMY_ORDER_SUMMARY.tax + DUMMY_ORDER_SUMMARY.service
  ...
  function handleProcessPayment() {
    ...
    // Simulasi loading hit API Payment Gateway selama 2 detik
    setTimeout(() => {
    ...
    return (
      <div className="mx-auto max-w-md min-h-screen bg-white flex flex-col items-center justify-center p-6 border-x border-neutral-200 text-center animate-in fade-in zoom-in-95 duration-300">
        ...
        <p className="text-xs text-neutral-400 mt-1 max-w-xs">
          Pesanan Anda sedang diteruskan ke dapur. Silakan tunggu di Meja {DUMMY_ORDER_SUMMARY.tableId}.
        </p>
        ...
        <div className="w-full bg-neutral-50 rounded-xl p-4 my-6 text-left border border-neutral-100 space-y-2">
          <div className="flex justify-between text-xs">
            ...
            <span className="font-mono font-medium text-neutral-700">{DUMMY_ORDER_SUMMARY.orderId}</span>
          </div>
          <div className="flex justify-between text-xs">
        ...
        <Button 
          ...
          onClick={() => router.push(`/customer?tenantId=${tenantId}`)}
        >
  ...
  return (
    <div className=" min-h-screen bg-neutral-50 flex flex-col relative border-neutral-200 shadow-lg">
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-neutral-100 p-4 flex items-center gap-3 shadow-sm">
        ...
        <div>
          ...
          <p className="text-[10px] text-muted-foreground">Meja {DUMMY_ORDER_SUMMARY.tableId} • {DUMMY_ORDER_SUMMARY.orderId}</p>
        </div>
      </header>
      ...
      <main className="flex-1 p-4 space-y-4 pb-28 overflow-y-auto">
        ...
        <div className="bg-white rounded-xl p-4 border border-neutral-100 shadow-sm space-y-3">
          ...
          <div className="space-y-2 text-xs divide-y divide-neutral-50">
            {DUMMY_ORDER_SUMMARY.items.map((item) => (
              <div key={item.id} className="flex justify-between pt-2 first:pt-0">
                <span className="text-neutral-600">
                  {item.name} <span className="text-neutral-400 font-medium text-[10px]">x{item.quantity}</span>
                </span>
            ...
            <div className="flex justify-between pt-2 text-neutral-500 text-[11px]">
              ...
              <span>{formatPrice(DUMMY_ORDER_SUMMARY.tax)}</span>
            </div>
            <div className="flex justify-between pt-2 text-neutral-500 text-[11px]">
              <span>Service Charge (5%)</span>
              <span>{formatPrice(DUMMY_ORDER_SUMMARY.service)}</span>
            </div>
          </div>
        </div>
